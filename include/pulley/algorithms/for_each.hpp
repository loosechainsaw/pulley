...
namespace pulley {
    namespace algorithms {
	    ...
	    struct for_each<std::tuple<T, Ts...>, F>
            {
		 static constexpr void apply(std::tuple<T, Ts...>& t, F f) noexcept(noexcept(f(std::get<0>(t))) && std::is_nothrow_copy_assignable<F>::value){
		     f(std::get<0>(t));
		     auto tt = tail(t);
		     detail::for_each<std::remove_reference_t<decltype(tt)>, F>::apply(tt, f);
	         }
	    };
#endif
...
#else
            detail::for_each<std::remove_reference_t<decltype(t)>, F>::apply(t, f);
#endif
        }
    }
}
